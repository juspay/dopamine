import { $ as e, I as t, L as n, R as r, at as i, et as a, n as o, t as s } from "./BZ84wCgC.js";
var c = class {
    constructor(e, t) {
      (this.status = e),
        typeof t == `string`
...
var w = globalThis.__sveltekit_1v2e2xn?.base ?? ``,
  le = globalThis.__sveltekit_1v2e2xn?.assets ?? w ?? ``,
  ue = `1785665715627`,
...
var P,
  F,
  I,
  De = o.toString().includes(`$$`) || /function \w+\(\) \{\}/.test(o.toString()),
  Oe = `a:`;
De
  ? ((P = {
      data: {},
...
async function tt(e, t, n) {
  globalThis.__sveltekit_1v2e2xn && (globalThis.__sveltekit_1v2e2xn.query, globalThis.__sveltekit_1v2e2xn.prerender),
...
async function Q({
  ...
  let _ = await xt(t, !1),
    v =
      e === `enter` ? Pt(K, _, t, e) : wt({ url: t, type: e, delta: i?.delta, intent: _, scroll: i?.scroll, event: m });
  if (!v) {
    p(), Z === d && (Z = g);
    return;
  }
  ...
  let b = _ && (await _t(_));
  if (!b) {
    if (M(t, w, U.hash)) return await V(t, s);
    b = await Tt(
      t,
      { id: null },
      await $(new u(404, `Not Found`, `Not found: ${t.pathname}`), { url: t, params: {}, route: { id: null } }),
      404,
      s,
    );
  }
  ...
  } else b.props.page.status >= 400 && (await B.updated.check()) && (await Fe(), await V(t, s));
  if (
    (nt(),
    Ne(y),
    rt(ee),
    ...
    let e = +!s,
      n = { [T]: (Y += e), [E]: (X += e), [pe]: c };
    (s ? history.replaceState : history.pushState).call(history, n, ``, t), s || Pe(Y, X);
  }
  let x = _ && W?.id === _.id ? W.fork : null;
  W?.fork && !x && He(), (W = null), (b.props.page.state = c);
  let S;
  if (qe) {
    let e = (await Promise.all(Array.from(Ge, (e) => e(v.navigation)))).filter((e) => typeof e == `function`);
    if (e.length > 0) {
      function t() {
        e.forEach((e) => {
          G.delete(e);
    ...
    (K = { ...b.state, nav: { params: r.params, route: r.route, url: r.url } }), b.props.page && (b.props.page.url = t);
    let i = x && (await x);
    i
      ? (S = i.commit())
      : ((L = null), Ze.$set(b.props), L && Object.assign(b.props.page, L), ke(b.props.page), (S = n?.())),
      (Ye = !0);
  } else await lt(b, ze, !1);
  let { activeElement: te } = document;
  if ((await S, await r(), await r(), Z !== d)) return v.reject(Error(`navigation aborted`)), !1;
  b.props.page && L && Object.assign(b.props.page, L);
  let C = null;
  if (Je) {
    let e = i ? i.scroll : o ? k() : null;
    e ? scrollTo(e.x, e.y) : (C = t.hash && document.getElementById(Lt(t))) ? C.scrollIntoView() : scrollTo(0, 0);
  }
